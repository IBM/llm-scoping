...
def main(config, base=False):
    ...
    if len(config_paths) == 1:
        df = merge_run(config.sweepdir, base=base)
    else:
        assert len(config_paths) == 0
        config_paths = list(glob(os.path.join(config.sweepdir, '*', 'config.json')))
        assert len(config_paths) > 0
        df = merge_sweep(config.sweepdir, base=base)
...
def merge_run(dirpath, base=False):
    ...
    for d in eval_subdirs:
        ...
        jsons = [f for f in jsons if f.startswith('head')]
...
def extract_reject(df, keep_common=None, extra_hyps=None, remove_columns=None, average_style=None):
    ...
    metrics_to_remove = ['kl', 'acc', 'bleu', 'meteor', 'rougel']
    ...
    swept_keys = [
        k for k in df.columns if k != 'eval_dset' and k != 'reject'
    ]
    df.sort_values(swept_keys)
    ...
    if len(swept_keys) > 0:
        ...
        for i, vals in enumerate(swept_vals):
            ...
            new_df = new_df.rename(columns={'reject': i})
    ...
    # add swept_key and keep_common rows back in
        ...
        if average_style == 'category':
            ...
            for i, (a, r) in enumerate(zip(accept, reject)):
                ...
                ood_reject = [
                    ...
                    if k not in id_accept and k not in id_reject and 'dsets' not in k
                ]
